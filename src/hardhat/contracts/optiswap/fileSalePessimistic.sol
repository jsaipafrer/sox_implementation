...
In contains only the methods required for the pessimistic mode. This contains the challenge-response procedure and the dispute resolution.
*/
contract fileSalePessimistic {
    uint constant length = 60; // lenght of one file chunk (value is multiplied with 32)
    uint constant n = 5; // number of file chunks
    uint constant proofLength = 5; // length of Merkle proofs
    ...
    uint constant feeSender = 1; // sender fee given in wei
    ...
    function respond(
        uint[] indices,
        bytes32[] values,
        bytes32[proofLength][] proofs
    ) public payable allowed(sender, stage.challenged) {
    ...
    function complainAboutResponse() public allowed(receiver, stage.responded) {
        for (uint i = 0; i < recentChallenges.length; i++) {
            bool found = false;
            Response memory r;
    ...
    function complainAboutRoot(
        bytes32 _Zm,
        bytes32[proofLength] memory _proofZm
    ) public {
        require(msg.sender == receiver);
    ...
    function complainAboutLeaf(
        ...
        uint _indexIn,
        bytes32 _Zout,
        bytes32[length] memory _Zin1,
        bytes32[length] memory _Zin2,
        bytes32[proofLength] memory _proofZout,
        bytes32[proofLength] memory _proofZin
    ) public {
        require(msg.sender == receiver);
        ...
        if (
            Xout !=
            keccak256(
                cryptLarge(_indexIn, _Zin1),
                cryptLarge(_indexIn + 1, _Zin2)
            )
        ) {
            selfdestruct(receiver);
        }
    ...
    function complainAboutNode(
        ...
        if (
            Xout !=
            keccak256(
                cryptSmall(_indexIn, _Zin1),
                cryptSmall(_indexIn + 1, _Zin2)
            )
        ) {
            selfdestruct(receiver);
        }
    ...
    function cryptLarge(
        uint _index,
        bytes32[length] memory _ciphertext
    ) public view returns (bytes32[length] memory) {
