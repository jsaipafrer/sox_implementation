...
type Gate = [number, number[]]; // [operationId, sons]
type Circuit = Gate[];
...
export async function evaluateCircuit(
    ...
    if (inputBlockSize <= 0) throw Error("Invalid block size");

    const numBlocks = Math.floor(input.length / inputBlockSize) + 1;
    ...
    evaluatedCircuit.push(input.slice(inputBlockSize * (numBlocks - 1))); // last block
